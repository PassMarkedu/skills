...
- 能选推理强度的应用（如 Codex）请选「高 / high」：中等强度实测会把评分标准里的正确答案当成卷面答案抄进证据。

## 使用

上传答卷扫描件（PDF 或按页顺序的照片），说「帮我批改这份试卷」。结果是一份可以直接转发的批改 PDF：首页总分与官方等级，每页答卷旁标分，每页后附批改说明和官方评分标准；需要核对的地方在对话里单独列出，不印在 PDF 上。暂无结构化评分标准的题不批改、不计入总分，批改开始前会先说明。对结果有异议时直接说「6(b) 应该给 1 分，因为……」，Skill 会重新生成，不再占用次数。
