...
- API base: `PASSMARK_BASE_URL` if set, otherwise `https://passmarkedu.com`. All calls below are relative to `<base>/api/v1/marking-kit`.
...
- The token lives in `~/.passmark/marking-kit-token` (create the folder if needed). If the home folder is not writable in this app's sandbox, use `.passmark-marking-token` in the current working folder instead, and check both places when looking for a saved token. Never print the token, never paste it into the chat, never ask the teacher for SMS codes or passwords.
...
"PassMark 阅卷 Skill 有新版本（<version>）。更新方法：把这句话发给我——『请从 <zip_url> 下载 PassMark 阅卷 Skill，解压后覆盖安装到原来的 passmark-marking 文件夹』；WorkBuddy 用户也可以在 SkillHub 里更新。"
...
- no saved token (first use in this app): "第一次使用需要连接你的 PassMark 账号。还没有账号的话，打开下面的链接后先注册（手机号即可），再点「授权」。"
...
2. Give the teacher the clickable link `verification_uri_complete` and the code: "打开链接 → 登录（或注册）PassMark → 核对授权码 <user_code> → 点「授权」。授权后回到这里，我会自动继续。"
...
4. Save `access_token` to the token file, tell the teacher "已连接 PassMark 账号", and continue with the task they asked for. Send the token as `Authorization: Bearer <token>` on every later call.

If a call returns 401 again right after a fresh login, stop and tell the teacher to contact PassMark support — do not loop.
...
- **No candidate** for a question → crop that question's area into a JPEG (under 8 MB) and call `POST /identify-photo` with `{"image_base64": "...", "mime_type": "image/jpeg"}`. This uses PassMark's image recognition and does not use the teacher's photo-search allowance. Use it only for questions the text step could not match.
...
- 404 → tell the teacher PassMark has no such paper and ask them to double-check the cover values.
- 429 with `"code": "quota_exceeded"` → say: "免费额度已用完（免费账号可批 1 份试卷）。订阅 PassMark 后可不限次使用：<base>/pricing" and stop.
...
| `numeric` | `value`: the student's final answer copied exactly as written (e.g. `"-0.945"`, `"3/8"`), and `present: true` if they wrote one | do not judge whether it is right — PassMark compares it |
...
When every question is judged, send the evidence JSON to `POST /score` (plain JSON body, same content you will submit; no PDF is made, nothing is charged again). The response has `recheck`: a short list of steps (low confidence, drawings, unclear dependencies, numbers PassMark could not compare, missing judgements, parts left unmarked) with the reason and the step's question.
...
curl -sS -X POST "$BASE/api/v1/marking-kit/results" \
  -H "Authorization: Bearer $(cat ~/.passmark/marking-kit-token)" \
